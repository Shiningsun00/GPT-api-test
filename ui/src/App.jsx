...
function WorkflowsPage({ api, agents, workflows, refresh, notify }) {
  ...
  const edit = (wf) => {
    ...
    setEditing(wf.id)
    setForm({
      id: wf.id, name: wf.name, mode: wf.mode,
      manager_agent_id: hierarchical ? wf.hierarchy?.manager_agent_id || '' : '',
      rows: hierarchical ? (wf.hierarchy?.workers || []) : (wf.steps || []).map((s) => ({ worker_id: s.step_id, agent_id: s.agent_id, additional_prompt: s.additional_prompt || '' })),
    })
  }
  const remove = async (id) => {
  ...
  return <div className="two-column wide-form">
    ...
    <form className="panel sticky" onSubmit={save}><div className="panel-title"><h3>{editing ? 'Workflow 수정' : '새 Workflow'}</h3>{editing && <Button type="button" tone="ghost" onClick={() => { setEditing(null); setForm(emptyWorkflow()) }}>취소</Button>}</div>
      ...
      {form.mode === 'hierarchical' && <Field label="Manager Agent"><select required value={form.manager_agent_id} onChange={(e) => setForm({ ...form, manager_agent_id: e.target.value })}><option value="">선택</option>{agents.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}</select></Field>}
      ...
      <div className="workers">{form.rows.map((row, index) => <div className="worker" key={`${index}-${row.worker_id}`}><input aria-label="slot id" value={row.worker_id} onChange={(e) => patchRow(index, { worker_id: e.target.value })} placeholder={form.mode === 'linear' ? 'step-1' : 'worker-1'} /><select required value={row.agent_id} onChange={(e) => patchRow(index, { agent_id: e.target.value })}><option value="">Agent 선택</option>{agents.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}</select><textarea rows="2" value={row.additional_prompt} onChange={(e) => patchRow(index, { additional_prompt: e.target.value })} placeholder="추가 지침 (선택)" /><button className="icon-button" type="button" aria-label="remove worker" onClick={() => setForm({ ...form, rows: form.rows.filter((_, i) => i !== index) })}>×</button></div>)}</div>
...
function RunStudio({ api, workflows, sessions, onRefreshSessions, notify }) {
  ...
  const [sessionData, setSessionData] = useState({ runs: [], messages: [], revisions: [], artifacts: [] })
  ...
  const loadSession = async (id = sessionId) => {
    if (!id) return
    try {
      const [runs, messages, revisions, artifacts] = await Promise.all([api.sessions.runs(id), api.sessions.messages(id), api.sessions.revisions(id), api.sessions.artifacts(id)])
      setSessionData({ runs, messages, revisions, artifacts })
  ...
  const start = async () => {
    ...
    setBusy(true)
    try { const outcome = await api.runs.start({ session_id: sessionId, user_request: prompt.trim() }); notify(`Run ${outcome.status}`); await loadSession(sessionId) } catch (error) { notify(error.message, 'error') } finally { setBusy(false) }
  }
  ...
  const submitFollowUp = async () => {
    ...
    setBusy(true)
    try { await api.sessions.followUp(sessionId, current.id, followUp.trim(), files); notify('Continuation Run이 완료되었습니다.'); setFollowUp(''); setFiles([]); await loadSession(sessionId) } catch (error) { notify(error.message, 'error') } finally { setBusy(false) }
  }
  ...
  return <div className="studio-grid">
    <div className="stack">
      ...
      <Card title="1. Session 선택 또는 생성"><div className="form-grid"><Field label="기존 Session"><select value={sessionId} onChange={(e) => setSessionId(e.target.value)}><option value="">선택</option>{sessions.map((s) => <option key={s.id} value={s.id}>{s.title || s.id}</option>)}</select></Field><Field label="새 Session Workflow"><select value={workflowId} onChange={(e) => setWorkflowId(e.target.value)}><option value="">선택</option>{workflows.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}</select></Field><Field label="Session 이름"><input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="LG엔솔 자기소개서" /></Field><div className="field button-field"><span>&nbsp;</span><Button onClick={createSession}>새 Session</Button></div></div></Card>
      <Card title="2. Initial Run"><Field label="사용자 요청"><textarea rows="6" value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="검증된 경험을 바탕으로 자기소개서를 작성해줘." /></Field><Button disabled={busy || !sessionId} onClick={start}>{busy ? '실행 중…' : 'Run 시작'}</Button></Card>
      {run && <Card title="3. Active / Latest Run" meta={`${run.run.id} · ${formatTime(run.run.updated_at)}`} actions={<StatusPill value={status} />}>
        <div className="metric-row"><div><span>Current stage</span><strong>{state.current_stage || '—'}</strong></div><div><span>Thread</span><strong>{run.thread_id}</strong></div><div><span>Checkpoints</span><strong>{history.length}</strong></div></div>
...
function HistoryPage({ api, sessions, notify }) {
  const [sessionId, setSessionId] = useState(sessions[0]?.id || '')
  const [data, setData] = useState({ runs: [], messages: [], revisions: [], artifacts: [] })
  const load = async () => {
    if (!sessionId) return
    try { const [runs, messages, revisions, artifacts] = await Promise.all([api.sessions.runs(sessionId), api.sessions.messages(sessionId), api.sessions.revisions(sessionId), api.sessions.artifacts(sessionId)]); setData({ runs, messages, revisions, artifacts }) } catch (error) { notify(error.message, 'error') }
  }
  useEffect(() => { load() }, [sessionId])
  return <div><div className="section-title"><div><span className="eyebrow">DURABLE RECORD</span><h2>History</h2><p>Session별 Run, Message, Artifact, Revision을 한곳에서 확인합니다.</p></div><select className="compact-select" value={sessionId} onChange={(e) => setSessionId(e.target.value)}><option value="">Session 선택</option>{sessions.map((s) => <option key={s.id} value={s.id}>{s.title || s.id}</option>)}</select></div>
    {!sessionId ? <Empty title="Session을 선택하세요" body="이력은 Workflow Session 단위로 보존됩니다." /> : <div className="history-grid"><Card title={`Runs · ${data.runs.length}`}>{data.runs.map((r) => <div className="record" key={r.id}><div><strong>{r.kind}</strong><small>{r.id}</small></div><StatusPill value={r.status} /></div>)}</Card><Card title={`Revisions · ${data.revisions.length}`}>{data.revisions.map((r) => <div className="record block" key={r.id}><strong>Revision v{r.version || r.revision}</strong><p>{r.final_output || 'Output persisted'}</p></div>)}</Card><Card title={`Messages · ${data.messages.length}`}>{data.messages.map((m) => <div className="record block" key={m.id}><strong>{m.role}</strong><p>{m.content}</p><small>Attachments {m.attachment_ids?.length || 0}</small></div>)}</Card><Card title={`Artifacts · ${data.artifacts.length}`}>{data.artifacts.map((a) => <div className="record block" key={a.id}><strong>{a.kind}</strong><small>{a.worker_id || a.worker_name || 'manager'}</small><p>{a.output}</p></div>)}</Card></div>}
  </div>
}

function SettingsPage({ apiBase, onSave, health }) {
  const [value, setValue] = useState(apiBase)
  return <div className="settings-wrap"><div className="section-title"><div><span className="eyebrow">LOCAL SETTINGS</span><h2>Settings</h2><p>UI 연결 설정만 저장합니다. OpenAI API key 같은 secret은 브라우저에 저장하지 않습니다.</p></div></div><Card title="Backend connection" actions={<StatusPill value={health === 'ok' ? 'CONNECTED' : 'DISCONNECTED'} />}><Field label="API Base URL" hint="기본 /api는 Vite local proxy를 통해 127.0.0.1:8000으로 연결됩니다."><input value={value} onChange={(e) => setValue(e.target.value)} placeholder="/api" /></Field><Button onClick={() => onSave(value)}>저장 및 재연결</Button></Card><Card title="Security boundary"><p>OpenAI API key는 backend 환경변수에서만 읽습니다. Local UI는 key 입력/저장 기능을 제공하지 않습니다.</p></Card></div>
}
...
export default function App() {
  ...
  const hydrate = async () => {
    try { await api.health(); setHealth('ok'); const [a, w, s] = await Promise.all([api.agents.list(), api.workflows.list(), api.sessions.list()]); setAgents(a); setWorkflows(w); setSessions(s) } catch { setHealth('down') }
  }
  ...
  return <div className="app-shell">
    ...
    <main className="content"><header className="topbar"><div><strong>Agent Workflow Studio</strong><span>Local-first · Single active run</span></div><Button tone="ghost" onClick={hydrate}>Sync</Button></header><div className="page">
      ...
      {page === 'settings' && <SettingsPage apiBase={apiBase} onSave={saveBase} health={health} />}
    </div></main>
